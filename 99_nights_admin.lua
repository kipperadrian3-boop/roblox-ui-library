...
main:CreateCheckbox("Show Admin Safe Zone", function(enabled)
...
local storyCoords = {
    { "[campsite] Main Camp Site", "0, 8, -0"},
    { "[safezone] Admin Safe Zone", "0, 110, -0" }
}

local storyDropdown = gametp:CreateDropDown("Location Teleports")
...
itemtp:CreateCheckbox("Item Visual Tracker (ESP)", function(state)
...
local dropdown = itemtp:CreateDropDown("Teleport to Specific Item")

for _, itemName in ipairs(itemNames) do
    dropdown:AddButton("Teleport to " .. itemName, function()
...
-- Jump Height Slider
plr:CreateSlider("Admin Jump Height", 700, 50, function(value)
...
plr:CreateSlider("Admin Movement Speed", 700, 16, function(value)
    _G.AdminWalkSpeed = value

    local function applyWalkSpeed(humanoid)
        if humanoid then
            humanoid.WalkSpeed = _G.AdminWalkSpeed
            humanoid.Changed:Connect(function(property)
                if property == "WalkSpeed" and humanoid.WalkSpeed ~= _G.AdminWalkSpeed then
                    humanoid.WalkSpeed = _G.AdminWalkSpeed
                end
            end)
        end
...
plr:CreateCheckbox("Speed Boost Toggle (50)", function(toggle)
    if toggle == true then 
        _G.AdminWalkSpeed = 50
    else
        _G.AdminWalkSpeed = 16
    end
...
--// VISUAL TOGGLES
vis:CreateCheckbox("Player Tracker (ESP)", function(state)
	ESPEnabled = state
	if not state then
		cleanupBillboardESP()
	else
...
vis:CreateCheckbox("Player Chams Visuals", function(state)
	ChamsEnabled = state
	if not state then
		cleanupChamsESP()
	else
...
vis:CreateCheckbox("Targeting FOV Circle", function(state)
...
local civDropdown2 = misc:CreateDropDown("Admin Console Tools", function() end)

civDropdown2:AddButton("Infinite Yield Admin Console", function()
    loadstring(game:HttpGet("https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source"))()
end)

civDropdown2:AddButton("Emote Utility GUI", function()
...
civDropdown2:AddButton("Anti-AFK Protection", function()
    wait(0.5)
...
-- Admin Perimeter Defense

local Workspace = game:GetService("Workspace")
local RemoteEvents = ReplicatedStorage:WaitForChild("RemoteEvents")

-- Admin Defense Variables
local adminDefenseToggle = false
local radius = 200
...
local function createDropdownWithCheckboxes(title, itemList, enabledTable)
    ...
    dropdown:AddCheckbox("Bulk Select All", function(checked)
        for _, itemName in ipairs(itemList) do
            enabledTable[itemName] = checked
        end
    end)
...
createDropdownWithCheckboxes("Auto Feed Campfire (Continuous)", campfireFuelItems, alwaysFeedEnabledItems)
createDropdownWithCheckboxes("Auto Feed Campfire (HP Based)", campfireFuelItems, autoFuelEnabledItems)
createDropdownWithCheckboxes("Auto Cook System", autocookItems, autoCookEnabledItems)
createDropdownWithCheckboxes("Auto Resource Grinder", autoGrindItems, autoGrindEnabledItems)
createDropdownWithCheckboxes("Auto Biofuel Converter", biofuelItems, autoBiofuelEnabledItems)

-- Auto Nourish
local eatDropdown = autofarmss:CreateDropDown("Auto Nourish (Timed Interval)")
eatDropdown:AddCheckbox("Enable Auto Nourish", function(checked)
    autoEatEnabled = checked
end)

-- Auto Nourish HP Based
local eatHPDropdown = autofarmss:CreateDropDown("Auto Nourish (Stat Bar Based)")
eatHPDropdown:AddCheckbox("Enable Auto Nourish (Stat Bar)", function(checked)
...
local miscdropdown = autofarmss:CreateDropDown("Environment Utilities")
miscdropdown:AddCheckbox("Gather Small Trees to Admin", function(checked)
...
local strongholdDropdown = main:CreateDropDown("Stronghold Admin Teleports")
...
strongholdDropdown:AddButton("Teleport to Stronghold Entrance", function()
    ...
    if targetPart then
        ...
        if destination and destination:IsA("BasePart") then
            ...
            if hrp then
                ...
                print("Teleported to Stronghold Entrance.")
            else
                warn("HumanoidRootPart not found!")
            end
        else
            warn("Destination part missing!")
        end
    else
        warn("Stronghold Entrance path not found!")
    end
end)
